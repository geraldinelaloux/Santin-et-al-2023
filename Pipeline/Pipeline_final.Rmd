...
popping_frames <- phase_intensity_summary |>
  ...
  mutate(
    # popping_frame = find_min(frame, for_which =is_local_peak & frame >= settings$Analysis_settings$Popping_point_phase$Minimum_frame), # to try
    popping_frame = min(which(is_local_peak & frame >= settings$Analysis_settings$Popping_point_phase$Minimum_frame)) # frame[which] is missing but here frame should be 1:last_frame so should be equivalent
    ) |> 
