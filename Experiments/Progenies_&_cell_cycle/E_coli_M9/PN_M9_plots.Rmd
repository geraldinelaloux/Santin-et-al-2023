...
```
```{r}
Progenies_segregation_timings |> filter(popping_frame < 240/8) |> select(id) |> unique() -> early_pop
Progenies_segregation_timings |> filter(area > 8) |> select(id) |> unique() -> high_area

plot_pixel_intensities(meshes |> filter(id %in% early_pop$id), channel = "phase", type = "wrap")
plot_pixel_intensities(meshes |> filter(id %in% high_area$id), channel = "diffuse", type = "wrap")
